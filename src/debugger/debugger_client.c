...
#define IO_READ_TIMEOUT 10000000
...
static ret_t debugger_client_dispatch_one(debugger_t* debugger, debugger_resp_t* resp) {
  ...
  ret_size = tk_istream_read_len(in, resp, sizeof(*resp), IO_READ_TIMEOUT);
  return_value_if_fail(ret_size == sizeof(*resp), RET_FAIL);

  assert(DEBUGGER_VERSION == resp->version);
  return_value_if_fail(debugger_client_extend_buff(debugger, resp->size) == RET_OK, RET_OOM);

  if (resp->size > 0) {
    ret_size = tk_istream_read_len(in, client->buff, resp->size, IO_READ_TIMEOUT);
    return_value_if_fail(ret_size == resp->size, RET_FAIL);
  }

  debugger_client_dispatch_message(debugger, resp);
...
static ret_t debugger_client_write_simple(debugger_t* debugger, uint32_t code, uint32_t data) {
  ...
  return tk_ostream_write_len(out, &req, sizeof(req), 1000) == sizeof(req) ? RET_OK : RET_FAIL;
}
...
static ret_t debugger_client_write_binary(debugger_t* debugger, uint32_t code, const void* data,
  ...
  req.size = size;
  req.version = DEBUGGER_VERSION;
  tk_ostream_write_len(out, &req, sizeof(req), 1000);

  return tk_ostream_write_len(out, data, size, 1000) == size ? RET_OK : RET_FAIL;
}
