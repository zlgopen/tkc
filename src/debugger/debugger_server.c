...
static ret_t debugger_server_write_data(tk_ostream_t* out, void* data, uint32_t size) {
  return tk_ostream_write_len(out, data, size, 2000) == size ? RET_OK : RET_IO;
}

static ret_t debugger_server_read_data(tk_istream_t* in, void* data, uint32_t size) {
  return tk_istream_read_len(in, data, size, 2000) == size ? RET_OK : RET_IO;
}
